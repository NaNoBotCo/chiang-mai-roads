...
def text_fields(rec: dict):
    ...
    for k in ("opened_note",):
        v = (rec.get("road") or {}).get(k)
        if v:
            yield f"road.{k}", v
    for k in ("method", "compare"):
        v = (rec.get("figure") or {}).get(k)
        if v:
            yield f"figure.{k}", v
    for i, s in enumerate((rec.get("figure") or {}).get("series") or []):
        if s.get("note"):
            yield f"figure.series[{i}].note", s["note"]
    for k in ("budget_note",):
        v = (rec.get("plan") or {}).get(k)
        if v:
            yield f"plan.{k}", v
    for k in ("gloss", "example_gloss"):
...
OFF_SUBJECT = re.compile(
    r"\b(a3\d\d|a380|boeing|airbus|fuselage|paintshop|football|basketball|volleyball|cricket|"
    r"motherboard|postage stamp|banknote|coat of arms|logo)\b", re.I)
ON_SUBJECT = re.compile(
    r"\b(chiang\s?mai|chiangmai|lanna|lampang|lamphun|chiang\s?rai|mae\s?hong\s?son|pai|fang|"
    r"thai|thailand|road|street|highway|motorway|bridge|moat|gate|soi|lane|alley|junction|"
    r"intersection|traffic|songthaew|tuk|bus|airport|railway|station|canal|river|ping|"
    r"suthep|nimman\w*|tha\s?phae|chang\s?klan|wua\s?lai|huay\s?kaew|wat|doi|map)\b", re.I)
